...
class RoomReminderLocalDataSource(
...
) : LocalAgendaRepository<Remainder> {

    override suspend fun getAgendaItemsById(agendaItemId: String): Remainder? {
        return reminderDao.getReminderById(reminderId = agendaItemId)?.toReminder()
    }

    override suspend fun getAgendaItemsByTime(time: Long): Flow<List<Remainder>> {
    ...
    override suspend fun upsertAgendaItem(agendaItem: Remainder): EmptyDataResult<DataError.Local> {
        return try {
    ...
    override suspend fun upsertAgendaItems(agendaItems: List<Remainder>): EmptyDataResult<DataError.Local> {
        return try {
    ...
    override suspend fun getAgendaItems(): Flow<List<Remainder>> {
